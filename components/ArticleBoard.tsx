...
import { updateStatus, updateAssignee } from '@/app/actions'
...
export default function ArticleBoard({ articles: initial, currentUser }: Props) {
  ...
  const visible = articles.filter(a => {
    if (a.funnel_stage !== activeStage) return false
  ...
  function handleStageChange(stage: FunnelStage) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Stage description ── */}
      <p className="text-xs text-stone-500 -mt-4">{STAGE_DESCRIPTIONS[activeStage]}</p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        />
        {PILLARS_BY_STAGE[activeStage].map(pillar => (
          <FilterPill
            key={pillar}
...
function ArticleRow({
  article, expanded, onToggle, onStatusChange, onAssigneeChange,
}: {
  article: Article
  expanded: boolean
  onToggle: () => void
